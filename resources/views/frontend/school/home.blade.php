                  ...
                  <div class="dash_wrpRLFt_C_tab">
                    <nav>
                      <div class="nav nav-tabs" id="student-tab" role="tablist">
                          @php
                              $uniqueStudents = $userData->student->unique(function($student) {
                                  return $student->student_name . '-' . $student->student_last_name;
                              });
                          @endphp
                          @foreach($uniqueStudents as $student)
                              <button class="nav-link @if($loop->first) active @endif" id="nav-{{ $student->id ?? '' }}-tab" data-bs-toggle="tab"
                                      data-bs-target="#nav-{{ $student->id ?? '' }}" type="button" role="tab" aria-controls="nav-{{ $student->id ?? '' }}"
                                      aria-selected="true">
                                  <h4>{{ $student->student_name ?? '' }} {{ $student->student_last_name ?? '' }}</h4>
              ...
              <div class="dash_wrpRRIt_z_A">
                ...
                <div class="dash_wrpRRIt_01 py-3 py-md-4 py-lg-5">
                  ...
                  <div class="tab-content" id="student-tabContent">
                    ...
                    @endphp
                    @foreach($uniqueStudents as $uniquestudent)
                    
                    <div class="tab-pane fade @if($loop->first) show active @endif" id="nav-{{ $uniquestudent->id ?? '' }}" role="tabpanel" aria-labelledby="nav-{{ $uniquestudent->id ?? '' }}-tab" tabindex="0">
                      @php
                      $students = App\Models\Students::where('student_name',$uniquestudent->student_name)->where('student_last_name',$uniquestudent->student_last_name)->orderBy('id','DESC')->get();
                      @endphp
                          ...
                          <div id="collapse_{{ $product->id ?? '' }}" class="accordion-collapse collapse @if($loop->first) show @endif" aria-labelledby="head_{{ $product->id ?? '' }}"
                            ...
                            <div class="accordion-body">
                              ...
                              <div class="abr_t">
                                ...
                                <div><input type="checkbox" class="digitalCheckbox" id="digitalCheckbox" value="{{ $product->digital_amount ?? '' }}"> {{ $product->digital_title ?? '' }} ( + {{ $product->digital_amount ?? '' }} AED )</div>
                                @endif
                                <div>&nbsp;</div>
                                <div class="d-flex">
...
<script>
  ...
  $(document).on('click', '.school-add-to-cart', function(e) {
    ...
    const checkboxes = document.querySelectorAll('.digitalCheckbox');
