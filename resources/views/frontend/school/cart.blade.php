      ...
      <div class="d-flex flex-wrap">
        ...
        <div class="insid_car_Lft">
          ...
          <div class="accordion" id="accordionCart">
            ...
            @if($cart->options->pkg_type == 'school')
            @php
              ...
              <h2 class="accordion-header" id="heading{{ $loop->iteration }}">
                ...
                <div class="accordionCart_Header">
                  ...
                  <div class="accordionCart_Head_EbLk accordionCart_Head_02">
                    <h4>{{ $product->title ?? '' }} </h4>
                    <p><strong>{{ $product->student->student_name ?? '' }} {{ $product->student->student_last_name ?? '' }} - {{ $product->student->classe ?? '' }}</strong></p>
                  </div>
...
<div class="accordionCart_Head_EbLk accordionCart_Head__Inc">

  <a type="submit" class="qty-down" id="{{$cart->rowId ?? ''}}" onclick="cartDecrement(this.id)"><span>-</span></a>

  <input type="text" name="quantity" class="qty-val" value="{{$cart->qty ?? ''}}" min="1">

  <a  type="submit" class="qty-up" id="{{$cart->rowId ?? ''}}" onclick="cartIncrement(this.id)"><span>+</span></a>

</div>


<div class="accordionCart_Head_EbLk accordionCart_Head_05">
  <h4>{{ $product->hard_copy_amount ?? '' }} AED</h4>
</div>
  ...
  <div class="accordion-body">
    <div class="cart_adnZ_Wrp">
      <div class="cart_adnZ">
        ...
        <div class="cart_adnZ_dTlz">
          <div class="cart_adnZ_dTlz01">
            <div class="accordionCart_Head_EbLk">
              @if($cart->options->digital_price > 0)
              <h4>Digital Price</h4>
              @endif
            </div>
            <!-- <p><i class="icon-date"></i>03 -03 2023</p> -->
          </div>
          <div class="cart_adnZ_dTlz02">
            <div class="accordionCart_Head_EbLk">
              @if($cart->options->digital_price > 0)
              <h4>{{ $product->digital_amount ?? '' }} AED</h4>
              @endif
            </div>
        ...
        <p>Collect from School - Collection date will be updated by school</p>
      </div>
    </div>
  </div>
</div>
...
@if($cart->options->pkg_type == 'service')
            ...
            $totprice = ($servicepackage->price + $optionprice + $addonprice);
            @endphp
            <div class="accordion-item" id="{{$cart->rowId ?? ''}}">
              <h2 class="accordion-header" id="heading{{ $loop->iteration }}">
                <button class="accordion-button" type="button" data-bs-toggle="collapse"
  ...
  <div class="insid_car_Ryt">
    <div class="insid_car_Ryt_payment">
      ...
      <div class="d-flex mt-5 pt-5 mb-0">
        <button id="paymentBtn" class="btn btn-primary mk_paymnt"><i class="icon-ion_card"></i><span>Make Payment</span><i
