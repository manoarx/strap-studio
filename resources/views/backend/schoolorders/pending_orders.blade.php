...
<section class="section">
  <div class="row">
    <div class="col-lg-12">
				...
				<div class="card">
					<div class="card-body">
						<h5 class="card-title">Pending Orders List</h5>
	...
	@foreach($orders as $key => $order)	
        ...
        @foreach($order->orderitem as $item)
			<tr>
				...
				<td>{{ $item->product->student->student_name ?? ''}} {{ $item->product->student->student_last_name ?? ''}}</td>
				<td>{{ $item->product->student->classe ?? ''}}</td>
				...
				<td>
					@if($order->status == 'pending')
			      	<a href="{{ route('admin.pending-confirm',$order->id) }}" class="btn btn-sm btn-success" id="confirm" >Confirm Order</a>
			      	@endif
<a href="{{ route('admin.schoolorder.details',$order->id) }}" class="btn btn-info btn-sm" title="Details">View Order</a>
<a href="javascript:void(0);" class="btn btn-danger btn-sm download_invoice" data-enid="{{ Crypt::encrypt($order->id) }}" data-id="{{ $order->id }}" title="Invoice Pdf">{{ invoiceButtonText($order->id) }} </a>
