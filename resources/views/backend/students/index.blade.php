    ...
    <div class="col-lg-12">
      <div class="card">
        <div class="card-body">
            ...
            <div class="row">
              <div class="form-group col-md-3">
                <select name="year" class="form-select mb-3" onchange="this.form.submit()">
                    ...
                    <option value="{{$yearlist->year ?? ''}}" @if($year==$yearlist->year) selected @endif>{{$yearlist->year ?? ''}}</option>
                    @endforeach
                </select>
              </div>
              <div class="form-group col-md-3">
                <select name="classe" class="form-select mb-3" onchange="this.form.submit()">
                    <option value="All">Select Classe</option>
                    @foreach ($classelist as $clist)
                    <option value="{{$clist->classe ?? ''}}" @if($classe==$clist->classe) selected @endif>{{$clist->classe ?? ''}}</option>
                    @endforeach
                </select>
              </div>
              <div class="form-group col-md-3">
                <input type="text" placeholder="Search..." name="searchkey" value=""> <input type="submit" name="submit"> <a href="{{ route('admin.students.index',['schoolid' => $schoolid]) }}" class="btn btn-sm btn-primary sendbulkemail">Clear</a>
              </div>
            </div>
          </form>
          <div class="">&nbsp;</div>
          ...
          <button class="btn btn-primary sendbulkemail">Send Bulk Email</button><button id="delete-selected" class="btn btn-danger">Delete Selected</button>
          <div class="">&nbsp;</div>
          ...
          <table class="table table-hover table-bordered table-striped" id="table">
            ...
            <tbody>
              ...
              <tr data-entry-id="{{ $student->id }}">
                <td><input type="checkbox" name="student[]" data-id="{{ $student->id }}" class="select-item" value="{{ $student->user->id }}"></td>
                ...
                @if($student->user->is_email_verified == 1)
                <button class="btn btn-sm btn-secondary">Verified</button>
                @else
                @if(count($student->user->userverify) > 0) 
                <button class="btn btn-sm btn-dark sendemail" data-user="{{ $student->user->id }}">ReSend Verification Mail</button>
                @else
                <button class="btn btn-sm btn-dark sendemail" data-user="{{ $student->user->id }}">Send Verification Mail</button>
                @endif
                @endif
                  <a class="btn btn-sm btn-info" href="{{ route('admin.students.edit', $student->id) }}">
                      Edit
                  </a>
                  <button type="button" class="btn btn-sm btn-primary" onclick="$('#file-upload-modal').modal('show'); $('#student-id').val({{ $student->id }}); $('#school-id').val({{ $student->school->id }});"> Add Product</button>
                  @if(count($student->products) > 0)
                  <a class="btn btn-sm btn-info" href="{{ route('admin.products.index',['studentid' => $student->id]) }}">
                      View Product
                  </a>
                  <a class="btn btn-sm btn-info" href="{{ route('admin.students.deleteproduct', $student->id) }}" id="deleteproduct">
                      Delete Product
                  </a>
                  @endif
                  <a class="btn btn-sm btn-danger" href="{{ route('admin.students.delete',$student->id) }}" id="delete" >Delete</a>
          ...
          Showing {{ $students->firstItem() }} to {{ $students->lastItem() }} of total {{$students->total()}}
